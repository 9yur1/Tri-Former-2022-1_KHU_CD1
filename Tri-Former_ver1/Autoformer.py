# New Decomp block(Residual 생성) 적용 및 Trend와 Residual에도 Auto-Correlation block 추가
...
class Model(nn.Module):
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec,
        ...
        # decoder input
        '''
        trend_init = torch.cat([trend_init[:, -self.label_len:, :], trend_init[:, -self.pred_len:, :]], dim=1) 
        seasonal_init = torch.cat([seasonal_init[:, -self.label_len:, :], seasonal_init[:, -self.pred_len:, :]], dim=1)  
        noise_init = torch.cat([noise_init[:, -self.label_len:, :], zeros], dim=1)
        '''
        trend_init = torch.cat([trend_init[:, -self.label_len:, :], mean], dim=1)
        seasonal_init = torch.cat([seasonal_init[:, -self.label_len:, :], zeros], dim=1)
        noise_init = torch.cat([seasonal_init[:, -self.label_len:, :],zeros], dim=1)
