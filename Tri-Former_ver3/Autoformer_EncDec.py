...
class series_decomp(nn.Module):
    ...
    def __init__(self, kernel_size):
        ...
        self.seasonal_decomp = seasonal_decomp(kernel_size, period=8)  #### period 값 변경하여 확인

    def forward(self, x):
        moving_mean = self.moving_avg(x)
        seasonal = self.seasonal_decomp(x - moving_mean)
        #seasonal = (x - moving_mean + seasonal)/2
...
class Decoder(nn.Module):
    ...
    def forward(self, x, cross, x_mask=None, cross_mask=None, trend=None, noise=None): # 여기도 수정
        
        for layer in self.layers:
            x, residual_trend, residual_noise = layer(x, cross, x_mask=x_mask, cross_mask=cross_mask, trend=trend, noise=noise) # 여기도 수정
             
            trend = self.norm(trend)
            trend = self.projection(trend)

            noise = self.norm(noise) # 여기도 수정
